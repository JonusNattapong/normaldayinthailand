...
class CurriculumRLModel(nn.Module):
    """
    Curriculum Reinforcement Learning Model
    ฝึกโมเดลจากงานง่ายไปยาก
    """
    def __init__(self, model_path, vocab_size, device='cuda'):
        super(CurriculumRLModel, self).__init__()
        self.device = device
...
def train_curriculum_rl(
    ...
    epochs=1,
    lr=1e-5,
    curriculum_schedule=[0.5, 0.3, 0.2]  # Percentages for different levels of difficulty
):
    """
    Train a Curriculum Reinforcement Learning model
    ...
    Args:
        ...
        lr: Learning rate
        curriculum_schedule: List of percentages for different levels of difficulty
    ...
    model = CurriculumRLModel(model_path, vocab_size, device).to(device)
    ...
    for epoch in range(epochs):
        ...
        for difficulty, percentage in enumerate(curriculum_schedule):
            num_batches = int(len(dataset) * percentage / batch_size)
            ...
            for i, batch in enumerate(dataset):
                ...
                if i % 10 == 0:
                    logger.info(f"Epoch {epoch+1}/{epochs}, Difficulty Level {difficulty+1}, Batch {i}/{num_batches}, "
    ...
    logger.info(f"Curriculum RL training complete. Model saved to {output_dir}")
