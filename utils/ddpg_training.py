...
class Actor(nn.Module):
    def __init__(self, base_model_path, vocab_size):
        ...
        self.actor_head = nn.Sequential(
            ...
            nn.Tanh()  # Output scaled to [-1, 1] for DDPG
        )
...
class Critic(nn.Module):
    def __init__(self, base_model_path, vocab_size):
        ...
        # Process state and action together
    ...
    def forward(self, input_ids, action, attention_mask=None):
        ...
        # Output Q-value
        q_value = self.critic_head(state_action)
        return q_value
...
def train_ddpg(
    ...
    """
    Train a DDPG (Deep Deterministic Policy Gradient) model for language generation
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(dataset):
            ...
            if len(replay_buffer) > batch_size:
                ...
                actor_loss = -critic(states_batch['input_ids'], actor_actions, states_batch['attention_mask']).mean()
    ...
    logger.info(f"DDPG training complete. Model saved to {output_dir}")
