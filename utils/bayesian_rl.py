...
class BayesianRLModel(nn.Module):
    """
    Bayesian Reinforcement Learning Model
    ใช้ Bayesian inference เพื่อจัดการกับความไม่แน่นอน
    """
    def __init__(self, model_path, vocab_size, device='cuda'):
        super(BayesianRLModel, self).__init__()
        self.device = device
...
def train_bayesian_rl(
    ...
    """
    Train a Bayesian Reinforcement Learning model
    ...
    model = BayesianRLModel(model_path, vocab_size, device).to(device)
    ...
    logger.info(f"Bayesian RL training complete. Model saved to {output_dir}")
